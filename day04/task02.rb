...
sleep_totals_by_guard = sleep_by_guard.transform_values do |ranges|
  ranges.each_with_object(Array.new(60, 0)) do |range, occurences|
    range.each { |element| occurences[element] += 1 }
  end
end

sleepy_guard, occurences = sleep_totals_by_guard.max_by { |guard, range| range.max }
